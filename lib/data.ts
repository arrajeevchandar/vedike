...
export async function getEvents(): Promise<PublicEvent[]> {
  ...
  const compRows = await db.select({ eventId: competitions.eventId }).from(competitions).where(inArray(competitions.eventId, rows.map((r) => r.id)));
  return rows.map((row) => ({
    ...row,
...
export async function getEvent(slug: string) {
  ...
  const comps = await db.select().from(competitions).where(eq(competitions.eventId, event.id)).orderBy(asc(competitions.startsAt));
...
export async function getCompetitions(): Promise<PublicCompetition[]> {
  ...
  const db = getDb();
  const rows = await db.select({ competition: competitions, event: events }).from(competitions).innerJoin(events, eq(competitions.eventId, events.id)).where(eq(events.publicationState, "PUBLISHED")).orderBy(asc(competitions.startsAt));
...
export async function getCompetition(slug: string): Promise<CompetitionDetail | null> {
  ...
  const db = getDb();
  const [row] = await db.select({ competition: competitions, event: events }).from(competitions).innerJoin(events, eq(competitions.eventId, events.id)).where(eq(competitions.slug, slug)).limit(1);
  if (!row) return null;
...
export async function getDashboardData() {
  ...
  const db = getDb();
  const [eventRows, compRows, subRows, paymentRows] = await Promise.all([
  ...
  return { events: eventRows, competitions: compRows, submissions: subRows, payments: paymentRows, realRevenuePaise: paymentRows.filter((p) => p.state === "COMPLETED").reduce((sum, p) => sum + p.amountPaise, 0) };
}
