...
export function createVedikeScene(container:HTMLElement){
  ...
  const gold=new THREE.Color("#F2B705"), red=new THREE.Color("#E63946"), cream=new THREE.Color("#FFF3DC");
  ...
  let target=0,p=0,raf=0,dead=false,start=performance.now();
  const tick=(now:number)=>{if(dead)return;raf=requestAnimationFrame(tick);const time=(now-start)/1000;p+=(target-p)*.075;const camZ=18-p*206;camera.position.set(Math.sin(time*.4)*.28+Math.sin(p*9)*1.2,4+seg(p,.24,.4)*1.6-seg(p,.44,.6)*1.4,camZ);camera.lookAt(0,3+seg(p,.24,.4)*4-seg(p,.44,.56)*4+seg(p,.68,.82)*1.5,camZ-26);
    const dawn=seg(p,0,.16);metroGeo.setDrawRange(0,Math.floor(metroCount*seg(p,.015,.17)));const rt=seg(p,.04,.2);rangoli.scale.setScalar(Math.max(.001,rt*(mobile?4:5.2)));rangoli.rotation.y=time*.12;rangoli.children.forEach((l,i)=>((l as THREE.Line).material as THREE.LineBasicMaterial).opacity=.85*rt*(1-seg(p,.3,.42)));lamps.forEach((s,i)=>s.material.opacity=.55*seg(p,.02+i*.008,.08+i*.008)*(1+.2*Math.sin(time*3+i)));city.visible=p<.4;
