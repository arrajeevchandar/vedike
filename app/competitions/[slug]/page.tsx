...
export default async function CompetitionPage({ params }: PageProps<"/competitions/[slug]">) {
  ...
  const showResults = competition.isShowcase || ["voting_open", "closing", "completed"].includes(phase);
  ...
  return <PublicShell><div className="shell page-pad">
    ...
    <div className="competition-layout" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(270px,340px)", gap: 40, alignItems: "start", marginTop: 14 }}>
      ...
      <aside className="glass" style={{ padding: 22, borderRadius: 20, position: "sticky", top: 92 }}>
        {showResults ? <><div className="eyebrow">Live leaderboard</div><h3 className="display" style={{ fontSize: 24, margin: "8px 0 18px" }}>{competition.voteCount.toLocaleString("en-IN")} votes</h3><LiveLeaderboard slug={slug} initial={competition.leaderboard.slice(0, 5)} /></> : <><div className="eyebrow">Community stage</div><h3 className="display" style={{ fontSize: 24, margin: "8px 0" }}>{phaseCopy[phase]}</h3><p className="muted">Entries stay private until the organiser opens public voting.</p></>}
